...
export interface RetainerPDF {
  base64: string;
  filename: string;
  size_bytes: number;
}

export async function generateRetainerPDF(extraction: ExtractionResult): Promise<RetainerPDF> {
  ...
  const incidentLabel: Record<string, string> = {
    vehicle_accident: "motor vehicle accident",
    slip_and_fall: "premises liability incident (slip and fall)",
    assault: "assault and battery incident",
    dog_bite: "animal attack incident (dog bite)",
    general_incident: "incident",
  };
  const incidentType = incidentLabel[reportType] || "incident";
  const isVehicleCase = reportType === "vehicle_accident";
  ...
  const drawCentered = (text: string, size: number, f = fontBold) => {
    ...
    page.drawText(text, { x: (pageWidth - w) / 2, y, size, font: f, color: rgb(0.06, 0.16, 0.26) });
    y -= size * 2;
  };
  ...
  const drawCenteredUnderline = (text: string, size: number) => {
    ...
    page.drawText(text, { x, y, size, font: fontBold, color: rgb(0.06, 0.16, 0.26) });
    page.drawLine({ start: { x, y: y - 2 }, end: { x: x + w, y: y - 2 }, thickness: 0.5, color: rgb(0.06, 0.16, 0.26) });
    y -= size * 2.5;
  };
  ...
  const drawParagraph = (text: string, f = font) => {
    ...
    for (const line of lines) {
      if (y < margin) addPage();
      page.drawText(line, { x: margin, y, size: fontSize, font: f, color: rgb(0, 0, 0) });
      y -= lineHeight;
    }
    y -= 8;
  };
  ...
  if (isVehicleCase) {
    ...
    drawParagraph(
      `The ${incidentType} giving rise to this Claim occurred at ${location}. The circumstances surrounding the incident, including the actions of the involved parties and any contributing factors, will be further investigated by Attorney as part of the representation under this Agreement.`
    );
  }
  drawParagraph(
    "Attorney is authorized to investigate the liability aspects of the incident, including the collection of police reports, witness statements, and documentation to determine the full extent of recoverable damages. Client understands that preserving evidence and providing truthful disclosures regarding the events leading to the loss are material obligations under this Agreement. This investigation will serve as the basis for identifying all applicable insurance coverage and responsible parties."
  );
  if (ad.num_injured > 0) {
    drawParagraph(
      `Additionally, since the ${incidentType} involved an injured person, Attorney will also investigate potential bodily injury claims and review relevant medical records to substantiate non-economic damages.`,
      fontItalic
    );
  } else {
    drawParagraph(
      `However, since the ${incidentType} involved no reported injured people, the scope of this engagement is strictly limited to the recovery of property damage and loss of use.`,
      fontItalic
    );
  }
  ...
  y -= 20;
  checkSpace(120);
  page.drawText("ACCEPTED BY:", { x: margin, y, size: fontSize, font: fontBold, color: rgb(0, 0, 0) });
  y -= 30;
  page.drawText("CLIENT ___________________________", { x: margin, y, size: fontSize, font });
  page.drawText("Date: _______________", { x: pageWidth - margin - 150, y, size: fontSize, font });
  y -= 18;
  page.drawText(clientName, { x: margin, y, size: fontSize, font: fontBold });
  y -= 30;
  page.drawText("Richards & Law Attorney ___________________________", { x: margin, y, size: fontSize, font });
  page.drawText("Date: _______________", { x: pageWidth - margin - 150, y, size: fontSize, font });
  y -= 18;
